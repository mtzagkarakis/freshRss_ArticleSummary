...
class ArticleSummaryExtension extends Minz_Extension
{
  ...
  public function addSummaryButton($entry)
  {
    ...
    $entry->_content(
      ...
      . '</div>'
      . '<div class="oai-summary-wrap collapsed" data-entry-id="' . $entry->id() . '">'
      . '<div class="oai-summary-header">'
      . '<h3 class="oai-summary-title" title="AI Summary & Q&A"></h3>'
      . '<button class="oai-collapse-btn" title="Expand"></button>'
      . '</div>'
      . '<div class="oai-summary-body">'
      . '<button data-request="' . $url_summary . '" class="oai-summary-btn"></button>'
      . '<div class="oai-summary-content"></div>'
      . '<button data-request="' . $url_question . '" class="oai-qa-button" title="Ask a Question"></button>'
      . '<div class="oai-chatbox">'
      . '<div class="oai-chat-messages"></div>'
      . '<div class="oai-chat-input-area">'
      . '<textarea class="oai-chat-input" placeholder="Ask a question about this article..." title="Ask a question about this article..." rows="2"></textarea>'
      . '<button class="oai-chat-send" title="Send"></button>'
      . '</div>'
      . '</div>'
      . '</div>'
      . '</div>'
      . '<div class="oai-article-controls">'
      . '<a href="' . htmlspecialchars($entry->link()) . '" target="_blank" rel="noopener noreferrer" class="oai-article-link oai-article-link-primary" title="Read Full Article" data-domain="' . htmlspecialchars(parse_url($entry->link(), PHP_URL_HOST)) . '"></a>'
      . '<button class="oai-readability-toggle" data-request="' . $url_readability . '" title="Toggle Reader Mode"></button>'
      . '</div>'
      . '</div>'
    );
    return $entry;
